...
const ChangeNameModal = ({ visibilityNameModal, setVisibilityNameModal }) => {
    ...
    const updateName = async (name) => {
        try {
            ...
            const response = await api.put(`customers/${id}/update`, { type: 'name', data: name.name });
            if (response.data) {
                updateUserInfo(response.data.data)
                setVisibilityNameModal(false)
                showToastWithGravity('Nome alterado com sucesso!')
            }
                                    ...
                                    <View style={styles.contentContainer}>
                                        ...
                                        <View style={{ flex: 1, flexDirection: 'row' }}>
                                            ...
                                            <TouchableOpacity
                                                ...
                                                onPress={() => setVisibilityNameModal(!visibilityNameModal)}
                                            //disabled={!isValid || isSubmitting}
                                            >
                                                <Text style={{ color: 'white' }}>
                                                    Voltar
