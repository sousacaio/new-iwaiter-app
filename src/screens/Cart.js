...
import {
    ...
    Modal, TouchableHighlight, TouchableOpacity, Button, Alert, StyleSheet, ScrollView,
} from 'react-native';
import { useSelector, useDispatch } from 'react-redux'
import { SafeAreaView } from 'react-native-safe-area-context';
import { addQuantity, subtractQuantity, removeItem, clearCart, storeOrderedItems } from './../actions/cartActions';
...
const Cart = (props) => {
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            <View style={{ flex: 1 }}>
                <View style={{ flex: 4 }} >
                    <TouchableOpacity style={styles.cleanCartButton}
                        onPress={() => { clearTheCart() }}    >
                    ...
                    <SafeAreaView style={{ flex: 1 }} >
                        {items.length > 0 ?
                            <FlatList
                                data={items}
                                renderItem={({ item, index }) => renderProducts({ item, index, removeCartItem })}
...
const HasOrderedArea = (props) => {
    ...
    return (
        <View style={styles.noOrdersYetContainer} >
            <View style={{ flex: 2 }}>
                <View style={{ flex: 1, }}>
                    <TouchableOpacity style={styles.noOrdersYetButton}
                        onPress={() => { props.setVisibilityMyOrders(true) }}>
                ...
                <View style={{ flex: 1, flexDirection: 'row' }}>
                    ...
                    <TouchableOpacity style={styles.noOrdersYetButton}
                        ...
                        <Text style={{ color: '#6200ee' }}>Carrinho(R$ {props.totalCart.toFixed(2)})</Text>
                    </TouchableOpacity>
                    {/* Aqui vai vir o total do carrinho + o q ja foi pedido */}
                    <TouchableOpacity style={styles.noOrdersYetButton}
                        onPress={() => { props.setVisibilityPaymentModal(true) }}
                    >
                        <Text style={{ color: '#6200ee' }}>Fechar(R$ {
            ...
            <View style={{ flex: 2 }}>
                {props.total === 0 ?
                    ...
                    <TouchableOpacity style={styles.noOrdersYetButton} onPress={() => { props.setModalVisible(true) }} >
                        <Text style={{ color: '#6200ee' }}>Checkout( R${props.totalCart})</Text>
                    </TouchableOpacity>
                }
            </View>
            <View style={{ flex: 1 }} >
